//! Local loot tracking: kill counts + item drops by zone/mob.
...
const RECENT_KILL_CAP: usize = 64;
...
pub struct LootEngine {
    data: LootFile,
    zone: Option<String>,
    recent_kills: VecDeque<RecentKill>,
    dirty: bool,
}
...
impl LootEngine {
    ...
    pub fn handle(&mut self, event: LogEvent, config: &AppConfig) -> bool {
        ...
        match event {
            ...
            LogEvent::Death { target, by_you } if by_you => {
    ...
    fn record_kill(&mut self, target: &str) {
        ...
        if mob.display_name.is_empty() {
            mob.display_name = display.clone();
        }
        mob.kills = mob.kills.saturating_add(1);

        self.recent_kills.push_front(RecentKill {
            mob_key: key,
            display,
            at: Utc::now(),
            coin_claimed: false,
        });
        while self.recent_kills.len() > RECENT_KILL_CAP {
            self.recent_kills.pop_back();
        }
        self.dirty = true;
    }
    ...
    fn record_item(&mut self, mob_raw: &str, item: &str, quantity: u32) {
        ...
        // Prefer this mob for upcoming coin lines.
        self.recent_kills.push_front(RecentKill {
            mob_key: key,
            display: mob.display_name.clone(),
            at: Utc::now(),
            coin_claimed: false,
        });
        while self.recent_kills.len() > RECENT_KILL_CAP {
    ...
    fn record_coin(&mut self, copper: u64) -> bool {
        let now = Utc::now();
        let idx = self.recent_kills.iter().position(|k| {
            !k.coin_claimed
                && now
                    .signed_duration_since(k.at)
                    .to_std()
        ...
        let kill = self.recent_kills[idx].clone();
        self.recent_kills[idx].coin_claimed = true;
